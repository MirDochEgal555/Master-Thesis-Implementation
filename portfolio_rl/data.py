    ...
    def _load_or_download_returns(self, cfg: YahooConfig) -> pd.DataFrame:
        if cfg.cache_path is not None:
            try:
                ...
                # assume cached is returns with columns tickers; reorder if possible
                if len(cfg.tickers) == 1 and df.shape[1] == 1:
                    df = df.rename(columns={df.columns[0]: cfg.tickers[0]})
                    return df
                if set(cfg.tickers).issubset(df.columns):
                    return df[cfg.tickers]
            except Exception:
                pass
        ...
        if isinstance(stock_data.columns, pd.MultiIndex):
            ...
            # reorder to requested ticker order
            if set(cfg.tickers).issubset(close_prices.columns):
                close_prices = close_prices[cfg.tickers]
            else:
                missing = [t for t in cfg.tickers if t not in close_prices.columns]
                raise ValueError(f"Missing tickers in downloaded data: {missing}")
        else:
            # single ticker case
            close_prices = stock_data[[cfg.price_field]].copy()
            close_prices.columns = cfg.tickers
